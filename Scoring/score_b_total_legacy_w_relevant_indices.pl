...
while(<INPUT>)
{
    ...
    $input =~ s/}//ig;
    $input =~ s/  / /ig;
    my @local_indices = [split(" ", $input)];
    push(@relevant_indices, @local_indices);
}
close(INPUT);
...
if(scalar @triangs ==1)
{
    $dual_sub = new fan::SubdivisionOfPoints(POINTS=>[@points[@{$relevant_indices[0]}]],MAXIMAL_CELLS=>$triangs[0]);
...
foreach(@a)
{
    ...
    if (scalar @triangs !=1)
    {
        $dual_sub = new fan::SubdivisionOfPoints(POINTS=>[@points[@{$relevant_indices[$_]}]],MAXIMAL_CELLS=>$triangs[$_]);
        $h1 = new Hypersurface<Min>(DUAL_SUBDIVISION=>$dual_sub);
    }
    if($ARGV[6] eq "True")
    {
        push(@homologies_array,$h1->PATCHWORK(SIGNS=>$signs[$_])->BETTI_NUMBERS_Z2);
        push(@score_array,sum(@{$homologies_array[-1]}));
        $h1->remove("PATCHWORK");
    }
    else
    {
        push(@score_array,sum(@{$h1->PATCHWORK(SIGNS=>$signs[$_])->BETTI_NUMBERS_Z2}));
        $h1->remove("PATCHWORK");
    }
}
