...
foreach(@a)
{
    ...
    if($ARGV[6] eq "True")
    {
        ...
        push(@score_array,$homologies_array[-1]->[0]+0.1*$homologies_array[-1]->[1]);
        $h1->remove("PATCHWORK");
    }
    else
    {
        push(@score_array,$h1->PATCHWORK(SIGNS=>$signs[$_])->BETTI_NUMBERS_Z2->[0]+0.1*$h1->PATCHWORK(SIGNS=>$signs[$_])->BETTI_NUMBERS_Z2->[1]);
        $h1->remove("PATCHWORK");
    }
}
