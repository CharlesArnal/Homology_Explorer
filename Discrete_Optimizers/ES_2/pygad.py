...
class GA:
    ...
    def cal_pop_fitness(self):
        ...
        # TODO MODIFIED HERE TODO
        # those that haven't been computed yet
        # Not super nicely coded, but some operations I wanted to perform weren't very numpy compatible 
        second_part_pop_and_indices = [[sol_index,sol] for sol_index, sol in enumerate(self.population ) if not ( (self.last_generation_parents is not None) and len(numpy.where(numpy.all(self.last_generation_parents == sol, axis=1))[0] > 0) )]
        second_part_pop = numpy.array([pair[1] for pair in second_part_pop_and_indices])
        second_part_indices = [pair[0] for pair in second_part_pop_and_indices]
        #WARNING will only work with my custom functions that take [batch_size,size_input] inputs
        second_part_fitness = numpy.array(self.fitness_func(second_part_pop, None))
        # those that were computed before
        first_part_pop_and_indices = [[sol_index,sol] for sol_index,sol in enumerate(self.population) if (self.last_generation_parents is not None) and len(numpy.where(numpy.all(self.last_generation_parents == sol, axis=1))[0] > 0)]
        first_part_pop = numpy.array([pair[1] for pair in first_part_pop_and_indices])
        first_part_indices = [pair[0] for pair in first_part_pop_and_indices]
        
        first_part_fitness = numpy.array([self.previous_generation_fitness[self.last_generation_parents_indices[numpy.where(numpy.all(self.last_generation_parents == sol, axis=1))[0][0]]] for sol in first_part_pop])
        
        correct_order = numpy.argsort(numpy.concatenate((first_part_indices, second_part_indices) ) if len(first_part_pop_and_indices) !=0 else second_part_indices).astype(int)
        pop_fitness = numpy.concatenate((first_part_fitness, second_part_fitness ))
        pop_fitness = pop_fitness[correct_order]
        """
        first_part_fitness = []
        ...
        #first_part_pop = [sol for _,sol in enumerate(self.population) if (self.last_generation_parents is not None) and len(numpy.where(numpy.all(self.last_generation_parents == sol, axis=1))[0] > 0)]
        #first_part_fitness = numpy.array([self.previous_generation_fitness[self.last_generation_parents_indices[numpy.where(numpy.all(self.last_generation_parents == sol, axis=1))[0][0]]] for sol in first_part_pop])
        
        # those that haven't been computed yet
        second_part_pop = numpy.array([sol for _, sol in enumerate(self.population ) if not ( (self.last_generation_parents is not None) and len(numpy.where(numpy.all(self.last_generation_parents == sol, axis=1))[0] > 0) )])
        # WARNING will only work with my custom functions that take [batch_size,size_input] inputs
        second_part_fitness = numpy.array(self.fitness_func(second_part_pop, None))
        print("first part population")
        print(numpy.array(first_part_pop))
        print("first part fitness ")
        print(first_part_fitness)
        print("second part population")
        print(second_part_pop)
        print("second part fitness ")
        print(second_part_fitness)
    ...
    def adaptive_mutation_population_fitness(self, offspring):
        ...
        # TODO MODIFIED HERE
        input = [temp_population[idx] for idx in range(len(parents_to_keep), fitness.shape[0]) ]
        results = self.fitness_func(numpy.array(input), None)
        fitness[len(parents_to_keep):fitness.shape[0]] = results
